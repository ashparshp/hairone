...
export default function AdminFinance() {
  ...
  const handleGenerateSettlements = async () => {
    ...
    try {
      ...
      const { cutoffDate, totalPayout, totalCollection, shopCount } =
        previewRes.data;
          ...
          },
          {
            ...
            onPress: async () => {
              try {
                ...
                Alert.alert(
                  "Success",
                  res.data.message || "Settlements generated.",
                );
      ...
      Alert.alert(
        "Error",
        e.response?.data?.message || "Failed to preview settlements.",
      );
      setGenerating(false);
    }
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={styles.header}>
        ...
        >
          {generating ? (
            <ActivityIndicator size="small" color={colors.actionPrimaryText} />
          ) : (
            <Text
              style={{
      ...
      <View style={[styles.tabs, { borderBottomColor: colors.tabBarBorder }]}>
        <TouchableOpacity
          style={[
            styles.tab,
            activeTab === "pending" && { borderBottomColor: colors.tint },
          ]}
        ...
        >
          <View style={{ flex: 1 }}>
            ...
            >
              {payout ? (
                <Text
                  style={{ color: colors.statusSuccess, fontWeight: "bold" }}
                >
                  Payout Pending
                </Text>
              ) : (
                <Text
                  style={{ color: colors.statusDanger, fontWeight: "bold" }}
                >
                  Collection Pending
                </Text>
              )}
            ...
            >
              {item.details.bookingCount} bookings live
            </Text>
          </View>
        </View>
      </FadeInView>
          ...
          <View style={{ alignItems: "flex-end" }}>
            <Text
              style={[
                styles.amount,
                {
                  color: isPayout ? colors.statusSuccess : colors.statusDanger,
                },
              ]}
            >
...
function HistoryDetailModalFixed({
  settlement,
  visible,
  onClose,
}: {
      ...
      >
        ...
        >
          <View style={{ alignItems: "center", marginVertical: 20 }}>
            ...
            >
              {isPayout ? (
                <DollarSign color={colors.statusSuccess} size={28} />
              ) : (
                <CreditCard color={colors.statusDanger} size={28} />
              )}
            </View>
            <Text
              style={{
            ...
            <Text
              style={{ color: colors.text, fontSize: 32, fontWeight: "bold" }}
            >
          ...
          >
            ...
            <Text style={{ color: colors.text, marginBottom: 8 }}>
              Shop: {settlement.shopId?.name}
            </Text>
            <Text style={{ color: colors.textMuted }}>
              Notes: {settlement.notes || "Auto-generated"}
            </Text>
            ...
            {!loadingDetails && details && (
              ...
              >
                ...
                >
                  <Text style={{ color: colors.textMuted }}>
                    Online Revenue (Held by Admin)
                  </Text>
                  <Text style={{ color: colors.statusSuccess }}>
                ...
                >
                  <Text style={{ color: colors.textMuted }}>
                    Cash Commission (Owed to Admin)
                  </Text>
                  <Text style={{ color: colors.statusDanger }}>
                    -₹{totalCashCommission.toFixed(2)}
                  </Text>
                </View>
                <View
                  style={{
                ...
                >
                  ...
                  <Text style={{ color: colors.text, fontWeight: "bold" }}>
                    {totalOnlineRevenue - totalCashCommission >= 0 ? "+" : "-"}₹
                    {Math.abs(totalOnlineRevenue - totalCashCommission).toFixed(
                      2,
                    )}
                  </Text>
                </View>
          ...
          <Text
            style={[
              styles.sectionTitle,
              { color: colors.textMuted, marginLeft: 20 },
            ]}
          >
            Included Bookings
          </Text>
          ...
          {loadingDetails ? (
            ...
            details.bookings.map((b: any, i: number) => (
              <View
                ...
                style={[
                  styles.rowCard,
                  { borderColor: colors.border, marginHorizontal: 20 },
                ]}
              >
                <View
                  style={{
                ...
                >
                  <Text style={{ color: colors.text, fontWeight: "bold" }}>
                    {format(new Date(b.date), "dd MMM")} • {b.startTime}
                  </Text>
          ...
          >
            ...
            >
              {processing ? (
                ...
                <Text
                  style={[styles.btnText, { color: colors.actionPrimaryText }]}
                >
                  Mark Payout Sent
                </Text>
              )}
...
const styles = StyleSheet.create({
  ...
  tabs: { flexDirection: "row", borderBottomWidth: 1, marginBottom: 20 },
  tab: {
    flex: 1,
    paddingVertical: 12,
    alignItems: "center",
