...
export default function ProfileScreen() {
  ...
  useEffect(() => {
    ...
    const fetchDraft = async () => {
      if (!user) return;
      if (user.role === "owner" && user.myShopId) {
        setIsDraftLoaded(true);
        return;
      }

      try {
        const res = await api.get("/admin/application-draft");
        if (!isMounted || !res.data) return;

        const draft = res.data.draft || {};
        const defaults = res.data.defaults || {};

        setOwnerName(draft.ownerName || defaults.ownerName || user.name || "");
        setBizName(
          draft.businessName ||
            draft.shopName ||
  ...
  useEffect(() => {
    ...
    if (!applying) return;
    if (!user) return;
    if (user.role === "owner" && user.myShopId) return;

    const trimmedOwner = ownerName.trim();
    const trimmedBusiness = bizName.trim();
    if (!trimmedOwner && !trimmedBusiness) return;

    const timer = setTimeout(async () => {
      try {
        setIsDraftSyncing(true);
        await api.put("/admin/application-draft", {
          ownerName: trimmedOwner,
          businessName: trimmedBusiness,
          shopName: trimmedBusiness,
        });
      } catch (e) {
        // Silent autosave failure; user can still submit explicitly.
      } finally {
  ...
  const handleApply = async () => {
    const cleanBusinessName = bizName.trim();
    ...
    try {
      const res = await api.post("/admin/apply", {
  ...
  const MenuItem = ({
    ...
    >
      <View
        style={[
          styles.menuIconBox,
          destructive && { backgroundColor: "rgba(255, 255, 255, 0.2)" },
        ]}
      >
      ...
      <View style={{ flex: 1 }}>
        <Text
          style={[
            styles.menuLabel,
            { color: destructive ? "white" : colors.text },
          ]}
        >
          {label}
        </Text>
        {subLabel && (
          <Text
            style={[
              styles.menuSubLabel,
              {
                color: destructive ? "rgba(255,255,255,0.8)" : colors.textMuted,
              },
            ]}
          >
            {subLabel}
      ...
      <ChevronRight
        size={16}
        color={destructive ? "white" : colors.textMuted}
      />
    </TouchableOpacity>
  );
    ...
    >
      ...
      <View style={[styles.header, { backgroundColor: colors.card }]}>
        ...
        >
          <Text
            style={[
              styles.roleText,
              { color: user?.role === "owner" ? "#0f172a" : colors.text },
            ]}
          >
      ...
      <FadeInView>
        <View style={styles.section}>
            ...
            <View style={[styles.promoCard, { backgroundColor: colors.tint }]}>
              {!applying ? (
                ...
                <View>
                  ...
                  {isDraftSyncing && (
                    <Text style={[styles.inputLabelDark, { marginTop: 0 }]}>
                      Saving draft...
                    </Text>
                  )}
          ...
          {(user?.applicationStatus === "rejected" ||
            user?.applicationStatus === "suspended") && (
            <View
              style={[
                styles.statusCard,
                ...
                {["male", "female", "other"].map((g) => (
                  <TouchableOpacity
                    ...
                    style={[
                      ...
                      {
                        backgroundColor:
                          editGender === g ? colors.tint : "transparent",
                        borderColor:
                          editGender === g ? colors.tint : colors.border,
                      },
                    ]}
      ...
      <Modal visible={logoutModalVisible} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          ...
          >
            <Text
              style={[styles.modalTitle, { color: colors.text, fontSize: 18 }]}
            >
              Sign out?
            </Text>
            <Text
