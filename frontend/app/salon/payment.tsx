  ...
  const fetchConfigAndLimits = async () => {
    try {
      const [configRes, limitRes] = await Promise.all([
        api.get("/shops/config").catch(() => null),
  ...
  const handleFinalize = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        paymentMethod:
          paymentMethod === "Pay Online (Test)" ? "ONLINE" : "CASH",
      };
      ...
      Alert.alert(
        "Booking Failed",
        "Slot might have been taken. Please try again.",
      );
      router.back();
    } finally {
      setLoading(false);
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        <Text style={[styles.heading2, { marginLeft: 16, color: colors.text }]}>
          Confirm & Pay
        </Text>
      </View>
              ...
              <View style={{ flexDirection: "row", alignItems: "center" }}>
                <View
                  style={[
                    ...
                    paymentMethod === m.id && { backgroundColor: colors.tint },
                  ]}
                >
                  <m.icon
                    size={20}
                    color={
                      paymentMethod === m.id ? "#020617" : colors.textMuted
                    }
                  />
                </View>
                <View>
                  <Text
                    style={[
                      styles.methodTitle,
                      { color: colors.text },
                      paymentMethod === m.id && { color: colors.tint },
                    ]}
                  >
                    {m.id}
                  </Text>
                  ...
                  {m.disabled && (
                    ...
                    >
                      Limit Reached (inc. cancellations)
                    </Text>
                  )}
                </View>
              </View>
          ...
          {config.userDiscountRate > 0 && (
            ...
            >
              <Text style={{ color: "#10b981" }}>
                Discount ({config.userDiscountRate}%)
              </Text>
              <Text style={{ color: "#10b981" }}>
