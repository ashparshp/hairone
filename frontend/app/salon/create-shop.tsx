...
export default function ManageServicesScreen() {
  ...
  const [shopType, setShopType] = useState<"male" | "female" | "unisex">(
    "unisex",
  );
  ...
  useEffect(() => {
    ...
    fetchShop();
  }, [user?._id, user?.myShopId]);

  useEffect(() => {
    if (!shop && user?.businessName) {
      setShopName((prev) => prev || user.businessName || "");
    }
  }, [shop, user?.businessName]);
  ...
  useEffect(() => {
    ...
    const payload: any = {
      ...
      minBookingNotice: Number(minNotice || 0),
      maxBookingNotice: Number(maxNotice || 30),
    };
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        await api.put("/admin/application-draft", payload);
      } catch (e) {
        // Silent autosave failure keeps manual save path unaffected.
      } finally {
  ...
  const fetchShop = async () => {
    ...
    if (!shopId) {
      try {
        const draftRes = await api.get("/admin/application-draft");
        const draft = draftRes.data?.draft || {};
        const defaults = draftRes.data?.defaults || {};
        ...
        if (draft.coordinates?.lat && draft.coordinates?.lng) {
          setCoords({ lat: draft.coordinates.lat, lng: draft.coordinates.lng });
        }
        if (draft.shopType) setShopType(draft.shopType);
        if (draft.bufferTime !== undefined)
          setBufferTime(String(draft.bufferTime));
        if (draft.minBookingNotice !== undefined) {
          setMinNotice(String(draft.minBookingNotice));
        }
        if (draft.maxBookingNotice !== undefined) {
          setMaxNotice(String(draft.maxBookingNotice));
        }
        if (typeof draft.autoApproveBookings === "boolean") {
          setAutoApprove(draft.autoApproveBookings);
        }
      } catch (e) {
    ...
    try {
      ...
      setMinNotice(
        s.minBookingNotice !== undefined ? String(s.minBookingNotice) : "60",
      );
      setMaxNotice(
        s.maxBookingNotice !== undefined ? String(s.maxBookingNotice) : "30",
      );
      ...
      if (shop && shop._id) {
        ...
        if (user && token) {
          login(token, {
            ...user,
            role: "owner",
            myShopId: newShop._id,
            businessName: shopName.trim() || user.businessName,
  ...
  const fetchLocation = async () => {
    try {
      ...
      if (geocode.length > 0) {
        const g = geocode[0];
        const newAddr =
          `${g.street || ""} ${g.city || ""}, ${g.region || ""} ${g.postalCode || ""}`.trim();
        setAddress(newAddr);
      } else {
          ...
          onPress: async () => {
            try {
              const res = await api.delete(
                `/shops/${shop._id}/services/${serviceId}`,
              );
              setShop(res.data);
              setServices(res.data.services);
      ...
      {shop && (
        ...
        >
          <TouchableOpacity
            style={[
              styles.tab,
              activeTab === "details" && { backgroundColor: colors.tint },
            ]}
            onPress={() => setActiveTab("details")}
          >
            <Store
              size={16}
              color={activeTab === "details" ? "black" : colors.textMuted}
            />
            <Text
              style={[
                styles.tabText,
          ...
          <TouchableOpacity
            style={[
              styles.tab,
              activeTab === "schedule" && { backgroundColor: colors.tint },
            ]}
            onPress={() => setActiveTab("schedule")}
          >
            <CalendarClock
              size={16}
              color={activeTab === "schedule" ? "black" : colors.textMuted}
            />
            <Text
              style={[
                styles.tabText,
          ...
          <TouchableOpacity
            style={[
              styles.tab,
              activeTab === "services" && { backgroundColor: colors.tint },
            ]}
            onPress={() => setActiveTab("services")}
          >
            <Scissors
              size={16}
              color={activeTab === "services" ? "black" : colors.textMuted}
            />
            <Text
              style={[
                styles.tabText,
                ...
                <View style={styles.row}>
                  <View style={{ flex: 1 }}>
                    ...
                    <Text
                      style={[styles.helperText, { color: colors.textMuted }]}
                    >
                      Booking blocked if less than this
                    </Text>
                  </View>
                  <TextInput
                    style={[
                ...
                <View style={styles.row}>
                  <View style={{ flex: 1 }}>
                    ...
                    <Text
                      style={[styles.helperText, { color: colors.textMuted }]}
                    >
                      Booking blocked if further than this
                    </Text>
                  </View>
                  <TextInput
                    style={[
                    ...
                    >
                      ...
                      >
                        <Scissors
                          size={20}
                          color={
                            item.isAvailable ? colors.tint : colors.textMuted
                          }
                        />
                      </View>
                      <View style={{ flex: 1 }}>
