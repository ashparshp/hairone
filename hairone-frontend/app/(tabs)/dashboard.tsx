...
import { 
  Plus, Clock, Settings, User, Calendar, Briefcase, 
  Scissors, MapPin, ChevronRight, Star, UserPlus 
...
export default function DashboardScreen() {
  ...
  const ActionCard = ({ icon: Icon, title, sub, onPress, color = Colors.primary }: any) => (
  ...
  const renderBarber = ({ item }: { item: any }) => (
    <TouchableOpacity 
      ...
      onPress={() => router.push({ pathname: '/salon/manage-barber', params: { barberId: item._id } } as any)}
    >
      <View style={styles.barberRow}>
        ...
        <View style={{flex:1}}>
           ...
           <View style={{flexDirection:'row', alignItems:'center', marginTop: 4, gap: 6}}>
             ...
             <Text style={styles.barberTime}>{item.startHour} - {item.endHour}</Text>
             {item.isAvailable ? (
                <View style={styles.statusBadge}><Text style={styles.statusText}>ON DUTY</Text></View>
             ) : (
                <View style={[styles.statusBadge, {backgroundColor: '#334155'}]}><Text style={[styles.statusText, {color: '#94a3b8'}]}>OFF</Text></View>
             )}
           </View>
        </View>
    ...
    <View style={styles.welcomeContainer}>
      <View style={styles.welcomeContent}>
          ...
          <Text style={styles.welcomeSub}>
              You have been approved. Create your digital storefront to start managing bookings and growing your business.
          </Text>
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
         <View>
             ...
             <View style={styles.locationRow}>
                 ...
                 <Text style={styles.shopLocation} numberOfLines={1}>{shop.address}</Text>
             </View>
         </View>
      ...
      >
          ...
          <View style={styles.teamHeader}>
             ...
             <TouchableOpacity style={styles.addBarberBtn} onPress={() => router.push('/salon/manage-barber' as any)}>
...
const styles = StyleSheet.create({
  ...
  ratingBox: { flexDirection: 'row', alignItems: 'center', backgroundColor: Colors.primary, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8, gap: 4 },
  ...
  // Action Cards
  actionCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: Colors.card, padding: 16, borderRadius: 16, borderWidth: 1, borderColor: Colors.border, gap: 16 },
  ...
  // Barber Card
  barberCard: { backgroundColor: Colors.card, padding: 16, borderRadius: 16, marginBottom: 12, borderWidth: 1, borderColor: Colors.border },
  ...
  editBtn: { marginLeft: 'auto', backgroundColor: '#334155', padding: 8, borderRadius: 8 },
  
  emptyState: { padding: 20, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: Colors.border, borderStyle: 'dashed', borderRadius: 12 },

  // Welcome / No Shop State
  welcomeContainer: { flex: 1, backgroundColor: Colors.background, justifyContent: 'center', padding: 20 },
  welcomeContent: { alignItems: 'center' },
  iconCircle: { width: 100, height: 100, borderRadius: 50, backgroundColor: 'rgba(245, 158, 11, 0.1)', alignItems: 'center', justifyContent: 'center', marginBottom: 24 },
  welcomeTitle: { fontSize: 32, fontWeight: 'bold', color: 'white', marginBottom: 12 },
  welcomeSub: { fontSize: 16, color: Colors.textMuted, textAlign: 'center', lineHeight: 24, marginBottom: 40 },
  createBtn: { backgroundColor: Colors.primary, paddingVertical: 18, paddingHorizontal: 40, borderRadius: 16 },
