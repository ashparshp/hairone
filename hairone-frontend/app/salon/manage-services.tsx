...
export default function ManageServicesScreen() {
  ...
  const isDark = theme === 'dark'; // Helper to identify dark mode
    ...
    <View style={[styles.container, {backgroundColor: colors.background}]}>
      ...
      <ScrollView contentContainerStyle={{paddingBottom: 40}} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
              ...
              {services.length === 0 ? (
                 ...
                 services.map((item, index) => (
                    <View key={index} style={[styles.serviceItem, {backgroundColor: colors.card, borderColor: colors.border}, item.isAvailable === false && {opacity: 0.6}]}>
                        {/* UPDATED: Dynamic background for service icon */}
                        <View style={[styles.serviceIcon, { backgroundColor: isDark ? colors.background : 'rgba(245, 158, 11, 0.1)' }]}>
                        ...
                        <View style={{flexDirection: 'row', alignItems: 'center', gap: 8}}>
                             <Switch
                                ...
                                thumbColor={item.isAvailable !== false ? (isDark ? "#000000" : "#ffffff") : "#94a3b8"}
                             />
                             {/* UPDATED: Dynamic background for action buttons */}
                             <TouchableOpacity onPress={() => startEditing(item)} style={[styles.actionBtn, {backgroundColor: isDark ? colors.border : '#e2e8f0'}]}>
            ...
            {/* UPDATED: Main form container uses colors.background for "True Black" */}
            <View style={[styles.addForm, {backgroundColor: isDark ? colors.background : '#f8fafc', borderColor: colors.border}]}>
                ...
                <View style={styles.inputGroup}>
                   ...
                   <TextInput 
                      // UPDATED: Dynamic background for input
                      style={[styles.formInput, {backgroundColor: isDark ? colors.background : '#ffffff', color: colors.text, borderColor: colors.border}]}
                ...
                <TouchableOpacity style={[styles.addBtn, {backgroundColor: colors.tint}]} onPress={handleAddOrUpdateService} disabled={addingService}>
                    {addingService ? <ActivityIndicator color="#000000"/> : (
                        <Text style={[styles.addBtnText, {color: "#000000"}]}>{editingServiceId ? 'Update Service' : 'Add Service'}</Text>
                    )}
                </TouchableOpacity>
            </View>
        </View>
