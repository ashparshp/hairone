    ...
    if (!isAdmin(req.user)) {
      if (req.user.applicationStatus !== "approved") {
        return res.status(403).json({
          message: "Application approval required before creating a shop",
        });
      }
...
exports.updateShop = async (req, res) => {
  try {
    ...
    if (!canManageShop(req.user, existingShop)) {
      return res
        .status(403)
    ...
    if (minBookingNotice !== undefined)
      updates.minBookingNotice = parseInt(minBookingNotice);
...
exports.getAllShops = async (req, res) => {
  try {
    ...
    if (search) {
      ...
      query.$or = [{ name: { $regex: regex } }, { address: { $regex: regex } }];
    }
    ...
    if (lat && lng) {
      ...
      shops = shops.map((shop) => {
        if (
          shop.coordinates &&
          shop.coordinates.lat !== undefined &&
          shop.coordinates.lng !== undefined
        ) {
      ...
      if (searchRadius && !search) {
        shops = shops.filter(
          (s) => s.distance !== null && s.distance <= searchRadius,
        );
      }
    ...
    const shopsWithSlots = await Promise.all(
    ...
    if (!lat || !lng) {
      filteredShops.sort((a, b) => {
        ...
        return (
          timeToMinutes(a.nextAvailableSlot) -
          timeToMinutes(b.nextAvailableSlot)
        );
      });
    }
...
// Helper for Home Screen Card Slot — shortest available service + overnight spillover
const findEarliestSlotForShop = async (shop, minTimeStr = "00:00") => {
  ...
  const availableServices = (shop.services || []).filter(
    (s) => s.isAvailable !== false && Number(s.duration) > 0,
  );
  const serviceDuration = availableServices.length
    ? Math.min(...availableServices.map((s) => Number(s.duration)))
    : 30;
  ...
  const bookingsMap = {};
  barbers.forEach((b) => {
    bookingsMap[b._id] = { today: [], yesterday: [] };
  });
  bookings.forEach((b) => {
    const entry = {
    ...
    const bucket = bookingsMap[b.barberId];
    if (!bucket) return;
    if (b.date === date) bucket.today.push(entry);
    else if (b.date === prevDate) bucket.yesterday.push(entry);
  });
  ...
  if (minStart >= maxEnd && maxEnd === 0) return null;

  const minFilter = timeToMinutes(minTimeStr);
  let current = Math.max(minStart, minFilter);

  const earliestAllowed =
    currentISTMinutes + (shop.minBookingNotice || 0) + bufferTime;

  if (current <= earliestAllowed) {
    const diff = earliestAllowed - current;
    const steps = Math.floor(diff / 15) + 1;
    current += steps * 15;
  }

  const isAvailableAt = (time) => {
    for (const barber of barbers) {
      ...
      if (today.isOpen) {
        const busyToday = [
          ...bBookings.today,
          ...bBookings.yesterday.map((r) => ({
            start: r.start - 1440,
            end: r.end - 1440,
          })),
        ];
        if (
          isBarberFree(today, time, serviceDuration + bufferTime, busyToday)
        ) {
          return true;
        }
      }
  ...
  while (current + serviceDuration <= maxEnd) {
    if (isAvailableAt(current)) return minutesToTime(current);
    current += 15;
  }
  return null;
};
...
exports.addBarber = async (req, res) => {
  ...
  try {
    ...
    if (!canManageShop(req.user, shop)) {
      return res
        .status(403)
        .json({ message: "Not authorized to manage this shop" });
    }
...
exports.updateBarber = async (req, res) => {
  ...
  try {
    ...
    const shop = await Shop.findById(existingBarber.shopId);
    if (!shop) return res.status(404).json({ message: "Shop not found" });

    if (!canManageShop(req.user, shop)) {
      return res
        .status(403)
        .json({ message: "Not authorized to manage this barber" });
    }
...
exports.getShopSlots = async (req, res) => {
  ...
  try {
    ...
    if (shop.isDisabled) {
      return res
        .status(403)
        .json({ message: "Shop is currently unavailable for booking" });
    }
    ...
    if (barberId && barberId !== "any") {
      ...
      if (b && b.shopId.toString() === shopId.toString()) barbersToCheck = [b];
    } else {
    ...
    const bookingsMap = {};
    barbersToCheck.forEach(
      (b) => (bookingsMap[b._id] = { today: [], yesterday: [] }),
    );
...
exports.updateShopService = async (req, res) => {
  ...
  try {
    ...
    if (duration !== undefined)
      updateQuery["services.$.duration"] = parseInt(duration);
    if (isAvailable !== undefined)
      updateQuery["services.$.isAvailable"] = isAvailable;
    ...
    if (!shop)
      return res.status(404).json({ message: "Shop or Service not found" });
    res.json(shop);
  } catch (e) {
    console.error(e);
...
exports.updateShopCombo = async (req, res) => {
  ...
  try {
    ...
    if (originalPrice !== undefined)
      updateQuery["combos.$.originalPrice"] = parseInt(originalPrice);
    if (duration !== undefined)
      updateQuery["combos.$.duration"] = parseInt(duration);
    if (items) updateQuery["combos.$.items"] = items;
    if (isAvailable !== undefined)
      updateQuery["combos.$.isAvailable"] = isAvailable;
...
exports.getShopRevenue = async (req, res) => {
  try {
    ...
    const shopCreatedDate = shop.createdAt
      ? new Date(shop.createdAt)
      : shop._id.getTimestamp();
